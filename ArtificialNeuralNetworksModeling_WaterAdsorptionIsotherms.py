...
# Standardizing the data (important for ANN)
...
ann_grid_search = GridSearchCV(ann, ann_param_grid, cv=5, scoring='neg_mean_squared_error', n_jobs=-1)
ann_grid_search.fit(X_train_scaled, Y_train)

# Best model and predictions
best_ann = ann_grid_search.best_estimator_
Y_pred_train = best_ann.predict(X_train_scaled)
Y_pred_test = best_ann.predict(X_test_scaled)
...
print(f"Best parameters: {best_ann}")
...
metrics_table.to_excel("Artificial Neural Networks modeling performance.xlsx")
...
for ax, label in zip(axs.flatten(), labels):
    ax.text(0.02, 0.95, label,
            ...
            va='top')
plt.tight_layout()
plt.savefig("ArtificialNeuralNetworksResults.tif", dpi=300, format="tif", bbox_inches="tight")
plt.show()